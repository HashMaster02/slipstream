...
// mkEvent builds a MarketEvent tagged with symbol so individual items can be
// told apart when verifying ordering.
func mkEvent(symbol string) events.MarketEvent {
	return events.MarketEvent{Type: events.MarketEventType, Symbol: symbol}
}

// popSymbol pops one event and asserts it is a MarketEvent, returning its symbol.
func popSymbol(t *testing.T, q *datastructures.Queue) string {
	...
	me, isMarket := got.(events.MarketEvent)
	if !isMarket {
		t.Fatalf("Pop() returned %T, want events.MarketEvent", got)
	}
	return me.Symbol
}
...
func TestQueue_PushPopSingle(t *testing.T) {
	...
	if me.Symbol != "AAPL" {
		t.Errorf("popped Symbol = %q, want %q", me.Symbol, "AAPL")
	}
	if got := q.Len(); got != 0 {
...
func TestQueue_PreservesEventValues(t *testing.T) {
	...
	in := []events.MarketEvent{
		{Type: events.MarketEventType, Symbol: "AAPL"},
		{Type: events.MarketEventType, Symbol: "MSFT"},
		{Type: events.MarketEventType, Symbol: "GOOG"},
	}
	for _, e := range in {
		q.Push(e)
	}
